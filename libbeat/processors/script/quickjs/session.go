...
type Session interface {
	// VM returns the Javascript runtime used for this session.
	VM() *quickjs.VM
...
func newSession(p *goja.Program, conf Config, test bool) (*session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create quickjs VM: %w", err)
	}
	// Setup JS runtime.
	s := &session{
		vm:             vm,
	...
	eventAtom, err := s.vm.NewAtom("Event")
	if err != nil {
		return nil, fmt.Errorf("failed to create Event atom: %w", err)
	}
	s.vm.GlobalObject().SetProperty(eventAtom, newBeatEventV0Constructor(s))
