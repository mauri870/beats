...
`
	...
	logFilePath := filepath.Join(filebeat.TempDir(), "log.log")
	filebeat.WriteConfigFile(fmt.Sprintf(beatsCfgFile, logFilePath, esAddr, fbOtelIndex))
	writeEventsToLogFile(t, logFilePath, numEvents)
	filebeat.Start()
	defer filebeat.Stop()
	...
	require.EventuallyWithT(t,
		func(ct *assert.CollectT) {
			...
			assert.Equal(ct, int64(0), bulkRetried, "expected no retries since there is no errors")
			assert.Equal(ct, int64(numEvents), bulkOK, "expected the number of successful bulk requests to match the number of events when bulk size is 1")
		},
